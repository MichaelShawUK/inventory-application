...
exports.club_update_post = [
  ...
  async (req, res, next) => {
    ...
    try {
      const old_club = await Club.findById(req.params.id);

      let imageUrl = old_club.image;

      if (old_club.image !== req.body.image) {
        imageUrl = await getImageUrl(req.body.image, "clubs");
      }

      updatedClub.image = imageUrl;

      await Club.updateOne({ _id: req.params.id }, updatedClub);
